...
class TrieNode {
 public:
   ...
   */
/**
 * @brief TrieNode的构造函数，用于初始化Trie节点
 * @param key_char 表示该节点对应的字符
 */
  explicit TrieNode(char key_char) {  // 使用explicit关键字防止隐式转换，确保只能显式调用构造函数
    key_char_ = key_char;  // 初始化节点的字符属性
  }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    std::unique_ptr<TrieNode> *target_node = root_->GetChildNode(*target_key_it);
    while (target_node != nullptr && target_key_it != key.end()) {
      target_node = target_node->get()->GetChildNode(*target_key_it);
      target_key_it++;
    }
    ...
    target_key_it = key.begin();
    TrieNode *parent_node = root_.get();
    while (target_key_it != key.end() - 1) {
        if (!parent_node->HasChild(*target_key_it)) {
    ...
    if (parent_node->HasChildren()) {
      ...
      std::unique_ptr<TrieNodeWithValue<T>> child_val_node = 
        std::make_unique<TrieNodeWithValue<T>>(std::move(*child_node), value);
  ...
  bool Remove(const std::string &key) { 
    ...
    while (target_node != nullptr && target_key_it != key.end()) {
      target_node = target_node->HasChild(*target_key_it) ? target_node->GetChildNode(*target_key_it)->get() : nullptr;
      target_key_it++;
    }
    if (target_node == nullptr || target_key_it != key.end()) {
      return false;
    ...
    target_key_it = key.end() - 1;
    while (true) {
      auto cur_key_it = key.begin();
      auto parent_node = root_.get();
      while (cur_key_it != target_key_it) {
        parent_node = parent_node->GetChildNode(*cur_key_it)->get();
        cur_key_it++;
      }
      auto child_node = parent_node->GetChildNode(*cur_key_it)->get();
      if (!child_node->HasChildren()) {
        parent_node->RemoveChildNode(*cur_key_it);
      }
      if (target_key_it == key.begin()) {
        break;
      }
      target_key_it--;
    }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    while (cur != nullptr && it != key.end()) {
      cur = cur->HasChild(*it) ? cur->GetChildNode(*it)->get() : nullptr;
      it++;
    }
